...
const EducationItem = memo(({ education, onEdit }: EducationItemProps) => {
  const { t } = useTranslation("ProfilePage");
...
const UserEducationsCardContent = memo(({ userId }: UserEducationsCardProps) => {
  ...
  const handleEdit = (id: string) => {
    routerEventEmitter.navigateToEditEducation(id);
  };
    ...
    <div className={styles.card}>
      ...
      <div className={styles.list}>
        {userEducations.map((education) => (
          <EducationItem key={education.id} education={education} onEdit={handleEdit} />
        ))}
      </div>
    </div>
  );
