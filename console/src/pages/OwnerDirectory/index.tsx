...
const OwnerDirectoryPage = memo(() => {
  ...
  return (
    <PageFrame>
      ...
      />
      {ownerError ? <EmptyState icon={ShieldCheck} title={(ownerError as Error).message || "Owner role required to list all profiles."} /> : null}
      <Flex direction="column" gap="4">
        <Card>
          <CardHeader icon={<ShieldCheck size={18} />} title="Forger" />
          {forgerItems.length === 0 ? (
            <EmptyState icon={ShieldCheck} title="No Forger profiles" />
          ) : (
            <Table.Root>
              <Table.Header>
                <Table.Row>
        ...
        <Card>
          <CardHeader icon={<ShieldCheck size={18} />} title="Authority" />
          {authorityItems.length === 0 ? (
            <EmptyState icon={ShieldCheck} title="No Authority profiles" />
          ) : (
            <Table.Root>
              <Table.Header>
                <Table.Row>
