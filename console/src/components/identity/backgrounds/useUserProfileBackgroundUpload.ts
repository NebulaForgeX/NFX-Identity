...
import type { UserProfileBackgroundDraft, UserProfileBackgroundUploadStatus } from "./drafts";
...
export function useUserProfileBackgroundUpload(profile: Profile.Response.ProfileBase, maxImages: number) {
  ...
  const moveDraft = (imageId: string, direction: -1 | 1) => {
    setDrafts((prev) => {
      const index = prev.findIndex((item) => item.imageId === imageId);
      if (index < 0) return prev;
      const next = index + direction;
      if (next < 0 || next >= prev.length) return prev;
      const active = prev[index];
      const over = prev[next];
      ...
      setDirty(true);
      return applyUserProfileBackgroundSortOrders(copy);
    });
  };
