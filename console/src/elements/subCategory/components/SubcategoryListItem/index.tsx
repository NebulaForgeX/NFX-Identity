  ...
  return (
    <article className={styles.subcategoryListItem} onClick={handleClick}>
      ...
      <div className={styles.contentContainer}>
        ...
        {subcategory.description && <p className={styles.description}>{subcategory.description}</p>}

        <div className={styles.metaRow}>
          <span className={`${styles.badge} ${subcategory.show ? styles.show : styles.hide}`}>
            {subcategory.show ? "显示" : "隐藏"}
          </span>
          {subcategory.parent && <span className={styles.parent}>父分类: {subcategory.parent.name}</span>}
          {subcategory.editor && (
