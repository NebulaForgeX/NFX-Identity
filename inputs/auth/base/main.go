...
func main() {
	...
	// === Init Logger ===
	if err := logx.Init(cfg.Logger, "auth-base-service", env.Env(*envStr)); err != nil {
	...
	if err := server.RunServer(ctx, cfg); err != nil && !errors.Is(err, context.Canceled) {
		logx.L().Fatal("auth server stopped with error", zap.Error(err))
	}

	logx.L().Info("auth server shutdown gracefully")
}
