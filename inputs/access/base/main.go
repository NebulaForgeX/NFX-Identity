...
import (
	...
	"nfxidentity/modules/access/server"
...
func main() {
	...
	// === Init Logger ===
	if err := logx.Init(cfg.Logger, "access-base-service", env.Env(*envStr)); err != nil {
	...
	if err := server.RunServer(ctx, cfg); err != nil && !errors.Is(err, context.Canceled) {
		logx.L().Fatal("access server stopped with error", zap.Error(err))
	}

	logx.L().Info("access server shutdown gracefully")
}
