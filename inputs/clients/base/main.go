...
import (
	...
	"nfxidentity/modules/clients/server"
...
func main() {
	...
	if err := logx.Init(cfg.Logger, "clients-base-service", env.Env(*envStr)); err != nil {
		log.Fatalf("logger init failed: %v", err)
	}
	defer logx.Sync()

	if err := server.RunServer(ctx, cfg); err != nil && !errors.Is(err, context.Canceled) {
		logx.L().Fatal("clients server stopped with error", zap.Error(err))
	}

	logx.L().Info("clients server shutdown gracefully")
}
