...
if nfxidentity_postgres_use_docker && [[ -z "${POSTGRES_CONTAINER_NAME:-}" ]]; then
...
ensure_postgresql_running() {
  ...
  if nfxidentity_postgres_use_docker; then
    if ! command -v docker &> /dev/null; then
      echo "Error: Docker command not found. Please install Docker first."
      exit 1
    fi
    output=$(nfxidentity_docker ps --filter "name=${POSTGRES_CONTAINER_NAME}" --format "{{.Status}}" 2>&1) || {
      echo "Error: Failed to check Docker container status."
      echo "Docker error output: ${output}"
      exit 1
    }
    if ! echo "${output}" | grep -q "Up"; then
      container_exists=$(nfxidentity_docker ps -a --filter "name=${POSTGRES_CONTAINER_NAME}" --format "{{.Names}}" 2>&1) || true
      if [ -z "${container_exists}" ]; then
        echo "Error: PostgreSQL container '${POSTGRES_CONTAINER_NAME}' not found."
        nfxidentity_docker ps -a --format "  - {{.Names}}" 2>&1 || true
      else
    ...
    if ! nfxidentity_psql_admin -c "SELECT 1" >/dev/null 2>&1; then
...
clean_shadow_connections() {
  ...
  nfxidentity_psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB_SHADOW}';" >/dev/null 2>&1
  sleep 2
  nfxidentity_psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB_SHADOW}';" >/dev/null 2>&1
}
...
clean_diff_database() {
  ...
  clean_shadow_connections
  result=$(nfxidentity_psql_admin -tc "SELECT 1 FROM pg_database WHERE datname = '${POSTGRES_DB_SHADOW}'" 2>&1)
  if echo "${result}" | grep -qE '\s*1\s*'; then
    echo "Dropping existing shadow database..."
    nfxidentity_psql_admin -c "DROP DATABASE ${POSTGRES_DB_SHADOW};" >/dev/null 2>&1
  fi
  echo "Creating shadow database..."
  create_result=$(nfxidentity_psql_admin -c "CREATE DATABASE ${POSTGRES_DB_SHADOW};" 2>&1)
  if [ $? -ne 0 ] && ! echo "${create_result}" | grep -qi "already exists"; then
    echo "Warning: Failed to create shadow database: ${create_result}"
  fi
  target_result=$(nfxidentity_psql_admin -tc "SELECT 1 FROM pg_database WHERE datname = '${TARGET_DB}'" 2>&1)
  if ! echo "${target_result}" | grep -qE '\s*1\s*'; then
    echo "Creating target database: ${TARGET_DB}"
    create_target_result=$(nfxidentity_psql_admin -c "CREATE DATABASE ${TARGET_DB};" 2>&1)
