...
import (
	...
	authconfig "nfxidentity/modules/auth/config"
...
func main() {
	...
	if _, err := os.Stat(".env"); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "run from NFX-Identity repo root (missing .env in cwd)\n")
		os.Exit(1)
	}

	cfg, err := authconfig.Load(ctx, clearEnv)
	if err != nil {
