...
nfxidentity_psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB_SHADOW}';" >/dev/null 2>&1 || true
nfxidentity_psql_admin -c "DROP DATABASE IF EXISTS ${POSTGRES_DB_SHADOW};" >/dev/null 2>&1 || true
nfxidentity_psql_admin -c "CREATE DATABASE ${POSTGRES_DB_SHADOW};" >/dev/null
