...
nfxidentity_normalize_pgsslmode() {
...
nfxidentity_effective_pgsslmode() {
  local s
  s=$(nfxidentity_normalize_pgsslmode)
  if [[ -n "$s" ]]; then
    printf '%s' "$s"
    return
  fi
...
nfxidentity_postgres_use_docker() {
...
nfxidentity_psql_admin() {
  if nfxidentity_postgres_use_docker; then
    ...
    nfxidentity_docker exec "${POSTGRES_CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d postgres "$@"
  else
    nfxidentity_ensure_psql_cli || return 1
    export PGPASSWORD="${POSTGRES_PASSWORD}"
    export PGSSLMODE="$(nfxidentity_effective_pgsslmode)"
