...
db_effective_sslmode() { nfxidentity_effective_pgsslmode; }
...
db_psql() {
  ...
  if nfxidentity_postgres_use_docker; then
    ...
    nfxidentity_docker exec -i "${POSTGRES_CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d "${dbname}" "$@"
  else
    nfxidentity_ensure_psql_cli || return 1
...
db_ensure_database() {
  local dbname="$1"
  local exists err
  exists="$(nfxidentity_psql_admin -tAc "SELECT 1 FROM pg_database WHERE datname = '${dbname}'" 2>/dev/null | tr -d '[:space:]')"
  if [[ "${exists}" == "1" ]]; then
    return 0
  fi
  echo "=== Database '${dbname}' does not exist — creating it ===" >&2
  if ! err="$(nfxidentity_psql_admin -v ON_ERROR_STOP=1 -c "CREATE DATABASE \"${dbname}\";" 2>&1)"; then
