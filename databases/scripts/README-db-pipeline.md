...
一套**不依赖 Atlas / Atlas Pro** 的并行流水线，直接连现有数据库，复用与 Atlas 相同的 shadow 库（`POSTGRES_DB_SHADOW` = `nfxidentity_diff`）。**不修改任何 Atlas 文件**——`atlas.hcl`、`run_atlas_pipeline.sh`、`templates/`、`scripts/gen_*.sh`、`atlas:*` task 原样保留。
...
和 Atlas 首次建库一样,`db_migrate_apply.sh` 和 `db_goose_baseline.sh` 在运行前会调用 `db_ensure_database`:**目标库(`POSTGRES_DB`,如 `nfxidentity_dev`)不存在时自动 `CREATE DATABASE`**(通过 `postgres` 维护库)。`run_db_pipeline.sh` 还会判断目标是否已初始化:
