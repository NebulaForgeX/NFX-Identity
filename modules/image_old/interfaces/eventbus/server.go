...
func NewServer(d Deps) (*Router, error) {
	...
	// 创建 registry
	registry := &Registry{
		Auth: handler.NewAuthHandler(d.ImageAppSvc()),
	}
...
func (r *Router) RegisterRoutes() {
	// 注册 Auth -> Image 事件处理器（Auth 服务通知 Image 服务）
	eventbus.RegisterHandler(r.EventRouter, r.registry.Auth.OnAuthToImage_ImageDelete)
	eventbus.RegisterHandler(r.EventRouter, r.registry.Auth.OnAuthToImage_ImageSuccess)
	eventbus.RegisterHandler(r.EventRouter, r.registry.Auth.OnAuthToImage_ImageTest)
}
