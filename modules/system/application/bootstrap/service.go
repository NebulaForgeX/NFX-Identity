...
type Service struct {
	...
	grpcClients       GRPCClients // gRPC 客户端（通过依赖注入）
}

// GRPCClients gRPC 客户端接口（定义在 application 层，实现由 infrastructure 层提供）
type GRPCClients interface {
	// Directory 服务客户端
	CreateUser(ctx context.Context, username string, status string, isVerified bool) (string, error) // 返回 user ID

	// Access 服务客户端
	CreateRole(ctx context.Context, key, name string, description *string, scopeType string, isSystem bool) (string, error) // 返回 role ID
	CreatePermission(ctx context.Context, key, name string, description *string, isSystem bool) (string, error)            // 返回 permission ID
	CreateRolePermission(ctx context.Context, roleID, permissionID string) (string, error)                                 // 返回 role_permission ID
	CreateGrant(ctx context.Context, subjectType, subjectID, grantType, grantRefID string, tenantID, appID *string) (string, error) // 返回 grant ID

	// Auth 服务客户端
	CreateUserCredential(ctx context.Context, userID, password string, tenantID *string, mustChangePassword bool) error
}
...
func (s *Service) initDirectoryService(ctx context.Context, cmd bootstrapCommands.BootstrapInitCmd) (uuid.UUID, error) {
	userIDStr, err := s.grpcClients.CreateUser(ctx, cmd.AdminUsername, "active", true)
	if err != nil {
...
func (s *Service) initAccessService(ctx context.Context, adminUserID uuid.UUID) (uuid.UUID, error) {
	...
	adminRoleID, err := s.grpcClients.CreateRole(ctx, "system.admin", "系统管理员", &adminRoleDesc, "global", true)
	if err != nil {
	...
	for _, perm := range permissions {
		...
		permID, err := s.grpcClients.CreatePermission(ctx, perm.key, perm.name, &permDesc, true)
		if err != nil {
	...
	for _, permID := range permissionIDs {
		_, err := s.grpcClients.CreateRolePermission(ctx, adminRoleID, permID)
		if err != nil {
	...
	// 4. 将角色分配给用户（通过 Grant）
	_, err = s.grpcClients.CreateGrant(ctx, "user", adminUserID.String(), "role", adminRoleID, nil, nil)
	if err != nil {
...
func (s *Service) initAuthService(ctx context.Context, userID uuid.UUID, password string) error {
	...
	if err := s.grpcClients.CreateUserCredential(ctx, userID.String(), password, nil, false); err != nil {
