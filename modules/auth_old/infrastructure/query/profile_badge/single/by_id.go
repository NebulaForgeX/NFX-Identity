...
// ByID 根据 ID 获取 ProfileBadge，实现 profileBadgeDomain.Single 接口
func (h *Handler) ByID(ctx context.Context, id uuid.UUID) (*profileBadgeDomainViews.ProfileBadgeView, error) {
	...
	if err := h.db.WithContext(ctx).Where("id = ?", id).First(&m).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, profileBadgeDomainErrors.ErrProfileBadgeViewNotFound
		}
		return nil, err
	}
	view := mapper.ProfileBadgeModelToDomain(&m)
	return &view, nil
}
