...
func (s *Service) BootstrapOwner(ctx context.Context, in BootstrapOwnerInput) (*BootstrapOwnerResult, error) {
	...
	exists, err := s.repoFactory.Profile(accountinternal.None()).Get.AnyOwnerExists(ctx)
	if err != nil {
		return nil, err
	}
	if exists {
	...
	if _, err := s.repoFactory.Email(accountinternal.None()).Get.ByEmail(ctx, emailAddr); err == nil {
		return nil, auth.ErrEmailAlreadyExists
	} else if !accountinternal.IsMissing(err) {
		return nil, err
	}
