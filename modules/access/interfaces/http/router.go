...
func (r *Router) RegisterRoutes() {
	...
	auth := access.Group("/auth", middleware.TokenAuth(r.tokenVerifier))
	{
		...
		auth.Get("/grants/:grant_id", r.handlers.Grant.GetByID)
		// 角色（key 路由在前，避免被 :role_id 匹配）
		auth.Post("/roles", r.handlers.Role.Create)
		auth.Get("/roles/key/:key", r.handlers.Role.GetByKey)
		auth.Get("/roles/:role_id", r.handlers.Role.GetByID)
		auth.Put("/roles/:role_id", r.handlers.Role.Update)
		auth.Delete("/roles/:role_id", r.handlers.Role.Delete)
		// 权限
		auth.Post("/permissions", r.handlers.Permission.Create)
		auth.Get("/permissions/key/:key", r.handlers.Permission.GetByKey)
		auth.Get("/permissions/:permission_id", r.handlers.Permission.GetByID)
		auth.Put("/permissions/:permission_id", r.handlers.Permission.Update)
		auth.Delete("/permissions/:permission_id", r.handlers.Permission.Delete)
		// 角色-权限关联
		...
		auth.Get("/role-permissions/:role_permission_id", r.handlers.RolePermission.GetByID)
		auth.Delete("/role-permissions/:role_permission_id", r.handlers.RolePermission.Delete)
		// Action
		auth.Post("/actions", r.handlers.Action.Create)
		auth.Get("/actions/key/:key", r.handlers.Action.GetByKey)
		auth.Get("/actions/:action_id", r.handlers.Action.GetByID)
		...
		auth.Get("/action-requirements/:action_requirement_id", r.handlers.ActionRequirement.GetByID)
		auth.Delete("/action-requirements/:action_requirement_id", r.handlers.ActionRequirement.Delete)
	}
}
